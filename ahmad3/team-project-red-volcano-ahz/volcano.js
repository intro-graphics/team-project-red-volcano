...
export class Volcano extends Volcano_Base {
    ...
    display(context, program_state) {
        ...
        //let model_transform9 = Mat4.scale(0.1, 0.1, 0.1).times(Mat4.translation(10 , 10 + t/(x*1000), 10));
        //velocity.push(model_transform9);

        for (let i = 0; i < 5; i++){
            //x +=  (Math.random() % 1);
            //this.shapes.smoke.draw(context, program_state, velocity[i], particles[i]);
            let model_transform9 = Mat4.scale(0.05, 0.05, 0.05).times(Mat4.translation(-3 + i, 10 + ((t/((velocity[i])*800))%20), 0));
            //if((10 + t/(velocity[i]*1000))< 20){
